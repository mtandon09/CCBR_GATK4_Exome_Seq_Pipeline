...
for(sample in sort(unique(c(somaliaDistance$V1,somaliaDistance$V2))))
{
  ...
  maxRelatedess<-which(samplePairs$V3 ==max(samplePairs$V3))
  maxHomCon<-which(samplePairs$V6 ==max(samplePairs$V6))
...
#verifyBAMID<-read.table("~/combineResults.txt",sep = "\t",header = T)
...
for(sample in sort(unique(c(verifyBAMID$Sample1,verifyBAMID$Sample2))))
{
  ...
  maxCorrelation<-which(samplePairs$VerfiyBAMId.Correlation ==max(samplePairs$VerfiyBAMId.Correlation))
