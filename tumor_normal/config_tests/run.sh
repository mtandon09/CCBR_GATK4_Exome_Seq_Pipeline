...
if [ ! -z $UNLOCK ]; then
    ...
    snakemake -j2 --unlock --snakefile tumor_normal_hg38.snakemake --rerun-incomplete
    exit 0
fi
...
if [ ! -z $RULEGRAPH ]; then
    ...
    snakemake --snakefile tumor_normal_hg38.snakemake --rerun-incomplete --rulegraph | dot -Tpng > $RULEGRAPH
    exit 0
fi
...
#smk_cmd_base="snakemake --stats snakemake.stats --restart-times 0 --rerun-incomplete -j 500 --cluster "$CLUSTER_OPTS" --cluster-config cluster.json --keep-going --snakefile tumor_normal_hg38.snakemake $untilarg $dryrunarg > snakemake.log 2>&1;"
smk_cmd_base="module load snakemake; snakemake --stats snakemake.stats --restart-times 0 --rerun-incomplete --cluster \"$CLUSTER_OPTS\" --cluster-config cluster.json --keep-going --snakefile tumor_normal_hg38.snakemake $untilarg"
...
if [ ! -z $DRYRUN ]; then
    ...
    if [ ! -z $LOCAL ]; then
        echo "module load snakemake; snakemake --rerun-incomplete --snakefile tumor_normal_hg38.snakemake -j $LOCAL $untilarg"
        eval "module load snakemake; snakemake --rerun-incomplete --snakefile tumor_normal_hg38.snakemake -j $LOCAL $untilarg"
    else
